...
public class Day4 {
    ...
    private static final class Range {
        ...
        public boolean overlapsWith(Range range) {
            if (this.to <= range.from && this.from <= range.to) return true;
    ...
    private static int part1(List<String> input) {
        ...
        for (String line : input) {
            ...
            if (range1.fullyContains(range2) || range2.fullyContains(range1)) count++;
        }
        return count;
    }
    ...
    private static int part2(List<String> input) {
        ...
        for (String line : input) {
            ...
            Range range1 = getRange(sections[0]);
            Range range2 = getRange(sections[1]);
            if (range1.overlapsWith(range2)) count++;
        }
        return count;
    }
